...
if [ ! -e '/usr/share/xray/xray' ]; then
    if [ $(arch) == aarch64 ]; then      wget -O /tmp/xray/Xray.zip https://kgithub.com/XTLS/Xray-core/releases/latest/download/Xray-linux-arm64-v8a.zip; fi
    if [ $(arch) == x86_64 ]; then     wget -O /tmp/xray/Xray.zip https://kgithub.com/XTLS/Xray-core/releases/latest/download/Xray-linux-64.zip; fi
    if [ $(arch) == armv7l ]; then     wget -O /tmp/xray/Xray.zip https://kgithub.com/XTLS/Xray-core/releases/latest/download/Xray-linux-arm32-v7a.zip; fi
