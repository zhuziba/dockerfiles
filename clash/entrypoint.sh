...
if [ ! -e '/root/.config/clash/dashboard/index.html' ]; then
    ...
    wget -t 0 -c https://kgithub.com/haishanh/yacd/releases/download/v0.3.8/yacd.tar.xz
    tar -xvf yacd.tar.xz
    ...
    mkdir -p /root/.config/clash/dashboard
    wget -t 0 -c https://download.fastgit.org/haishanh/yacd/releases/download/v0.3.8/yacd.tar.xz
    tar -xvf yacd.tar.xz
    mv /public/* /root/.config/clash/dashboard
    rm -rf /yacd.tar.xz
fi
...
if [ ! -e '/root/.config/clash/Country.mmdb' ]; then
    ...
    wget -t 0 -c -P /root/.config/clash https://download.fastgit.org/Loyalsoldier/geoip/releases/latest/download/Country.mmdb
    else
...
if [ ! -e '/root/.config/clash/smartdns.conf' ]; then
    ...
    if [ $(arch) == aarch64 ]; then      wget -t 0 -c -O /tmp/smartdns https://download.fastgit.org/pymumu/smartdns/releases/latest/download/smartdns-aarch64; fi
    if [ $(arch) == x86_64 ]; then     wget -t 0 -c -O /tmp/smartdns https://download.fastgit.org/pymumu/smartdns/releases/latest/download/smartdns-x86_64; fi
